...
### Local / Développement
Sur la machine souveraine (MIDGARD), la clé est configurée dans l'environnement local `~/.bashrc` ou `.env`.

Note: La clé n'est pas injectée en CI aujourd'hui ; si elle le devient, son absence produit 66 (fail-closed).
```bash
