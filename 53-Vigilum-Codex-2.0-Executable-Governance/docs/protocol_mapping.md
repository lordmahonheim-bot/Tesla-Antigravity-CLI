...
| **Gate 2 Délégation (SPINOFF-DIAG-GATE2-BYPASS)** | `core/orchestration/gate2_guard.py` (`issue-token` / `pre-flight` / `consume` / `delegate` / `release` / `status`) | jeton `G2T-1` HMAC-SHA256, secret hors workspace (`TESLA_GATE2_SECRET` / `~/.tesla/gate2/secret.key` 0600) | Pré-vol **pur** idempotent + liaison (mission, `graph_sha256`, fenêtre, nonce) + anti-rejeu `O_CREAT\|O_EXCL` (A-003) + **Transaction Safe-Spawn A-7** (`RESERVE→SPAWN→OBSERVE` : COMMIT_SUCCESS / COMMIT_FAILURE / ABORT_SAFE / UNKNOWN_CONFINED ; `release` manuel signé pour verrou RESERVED non observé) + grand livre chaîné `runtime/gate2/redemptions.jsonl` (`GATE2_LEDGER_CHAIN_BROKEN` si falsifié) |
