...
> **Principe Directeur Définitif (V2.6.3 — plan consolidé V2.6.2, SPEC LOCK) :** *« L'IA propose. L'outil déterministe vérifie. La preuve est dérivée d'observations contrôlées. Le contrôleur détermine l'état. L'OS restreint les capacités. L'autorité humaine engage. »*
> **Honnêteté sémantique (C1) :** les hooks et `TESLA_AGENT_IDENTITY` sont des garde-fous **sur leurs chemins d'exécution** (*execution-path guards*) — non des verrous physiques infranchissables de l'OS. La restriction universelle des privilèges relève de la séparation UID/GID du système (OI-03).
> **Honnêteté cryptographique (C2) :** un HMAC (clé symétrique) est une **attestation locale** — quiconque peut vérifier peut aussi forger. Son niveau de confiance est explicitement borné par l'isolation du signataire et la protection du matériel de clé ; l'indépendance de tiers véritable exige une signature asymétrique (courtier Ed25519 / OI-03, ou GPG/SSH selon A-002). Aucune attestation HMAC n'est présentée comme une signature indépendante de tiers.
...
11. **P11 — ASSERTION ≠ EVIDENCE (V2.6.1) :** Toute affirmation narrative concernant une preuve (« N/N tests passent », « déployé », « validé ») ne constitue pas une preuve. Seul un artefact d'exécution vérifiable — ledger signé, empreinte, quittance — fait foi ; un rapport déclaratif non corrélé à un registre physique est réputé non validé. Enforcement déterministe : **Gate R** (Evidence Reconciliation, `bin/gate_r.py`) avant l'état `MARBLE_ELIGIBLE`.
    **Extension V2.6.3 (SPEC LOCK) — échelle épistémique à cinq fonctions :** *ASSERTION ≠ OBSERVATION ≠ VALIDATION ≠ ATTESTATION ≠ AUTHORIZATION.* L'**assertion** est un récit ; l'**observation** est une donnée brute capturée (sortie de tests, ligne de transcript) ; la **validation** est un contrôle déterministe contre critères (runner ↔ manifeste) ; l'**attestation** est un énoncé signé liant des observations (enveloppe DSSE — attestation *locale* au sens C2) ; l'**autorisation** est une permission d'agir (jeton consommable, transition d'état, engagement humain). Confondre deux niveaux de l'échelle est une violation de P11.
...
* **V2.6.3 (SPEC LOCK) :** Verdict du Plan Consolidé V2.6.2 — **ratifié et scellé**. Le plan adopte l'intégralité des arbitrages précédents (nomenclature 07/08/09/10, défense en profondeur, détection de spoofing, Gate 0 sans artefact, registre dérivé, anti-friction v2) et apporte quatre raffinements d'honnêteté doctrinale, gravés : **Principe Directeur Définitif** (chaîne complète : proposition → vérification → preuve dérivée d'observations contrôlées → contrôle d'état → restriction OS → engagement humain) ; **C1** (garde-fous sur chemins d'exécution, non verrous OS) ; **C2** (HMAC = attestation locale symétrique, confiance bornée par l'isolation du signataire et la protection de clé — jamais présentée comme signature indépendante de tiers) ; **P11 étendue** (échelle ASSERTION ≠ OBSERVATION ≠ VALIDATION ≠ ATTESTATION ≠ AUTHORIZATION). Le **SPEC LOCK déclaré par le plan est matérialisé physiquement** via `bin/audit_cap.py` (L-001) : cycle de consolidation V2.6 verrouillé après 3 passes d'audit, preuve durable dans `evidence/spec_lock_*.json`. Le cycle V2.5.0→V2.6.2 est clos : toute évolution ultérieure exige une nouvelle mission par la FSM, jamais une révision de ce plan.
