...
| **Gate 2 Delegation Guard (2.1.3)** | `core/orchestration/gate2_guard.py` | Intercepteur pré-vol **pur** (idempotent) de `invoke_subagent` : jeton humain `G2T-1` signé HMAC (secret hors workspace), lié à (mission, `graph_sha256`, fenêtre, nonce), consommation atomique `O_CREAT\|O_EXCL` (A-003) + **Transaction Safe-Spawn `delegate`** (RESERVE → SPAWN → OBSERVE, A-7) + **`release` manuel signé** (reprise post-crash) + grand livre d'échange chaîné SHA-256 (`runtime/gate2/redemptions.jsonl`). RETEX `SPINOFF-DIAG-GATE2-BYPASS`. |
...
[SPINOFF-DIAG-GATE2-BYPASS — Gate 2 Delegation Guard, 2026-09-02]
  Python suite (governance + RETEX + gate2_guard): 81/81 PASS
    ...
    — Transaction Safe-Spawn A-7 (delegate: RESERVE→SPAWN→OBSERVE, release signé)
    — Bypass Testing: 9/10 vecteurs TESTÉS (BYPASS-01 → chantier A-4)
  Bash hook suite: 11/11 PASS
  Manifeste déclaratif (arbitrage #5): déclaré 92 = exécuté 92 — écart 0
TOTAL TESTS (V2.0 + V2.1.3 + Gate2 Guard + A-7): 92 | PASSED: 92 | FAILED: 0
======================================================================
```
...
| Gate 2 Délégation (jeton humain) | `python3 core/orchestration/gate2_guard.py pre-flight|consume|delegate|release --graph <f> --mission <id> --secret-file <key>` | jeton absent/falsifié/rejoué → exit 1 ; spawn inconnu (timeout) → 66 UNKNOWN_CONFINED ; secret non observable → exit 66 (P3) |
