...
Tesla-Web-Raider n'automatise pas des clics.

Il recherche toujours le niveau d'interaction le plus sémantique disponible.

Les clics, sélecteurs CSS et scripts Playwright constituent un dernier recours lorsque les interfaces plus expressives ne sont pas disponibles.
---
...
### 2.9. Native AI Web Interfaces (Semantic Interaction Layer)

Tesla-Web-Raider privilégie toujours le niveau d'interaction le plus robuste, le plus léger et le moins dépendant de scripts fragiles.

Lorsqu'un site expose une interface IA native (PageAgent ou technologie équivalente), cette interface devient une couche d'interaction privilégiée avant toute automatisation traditionnelle.

Ordre de priorité :

API officielle
→ Flux structurés (RSS, JSON, XML)
→ Interface IA native (PageAgent, Semantic UI)
→ DOM structuré
...
- réduire la complexité des scripts d'automatisation ;
- diminuer les risques de rupture lors d'une évolution de l'interface graphique ;
- privilégier des interactions exprimées en langage naturel lorsque le site les supporte ;
- diminuer la consommation CPU et mémoire ;
- réduire la quantité de contexte envoyée au LLM ;
- améliorer la robustesse des workflows de navigation.

Tesla-Web-Raider détecte automatiquement la présence d'une couche IA native et décide dynamiquement du meilleur mode d'interaction.

Si aucune interface sémantique n'est disponible, le Skill revient automatiquement au pipeline Webwright sans dégradation fonctionnelle.

Les interfaces IA natives sont considérées comme une optimisation de haut niveau et jamais comme une dépendance. Tesla-Web-Raider doit conserver une compatibilité complète avec l'ensemble du Web.
...
### 4.1. Semantic-first Navigation

Lorsque plusieurs méthodes permettent d'atteindre le même objectif, Tesla-Web-Raider applique systématiquement le principe :

"Le niveau d'abstraction le plus élevé capable d'accomplir correctement la tâche."

Ordre de préférence :

1. API
2. Commandes sémantiques
3. DOM
4. Browser Automation
5. OCR

Cette hiérarchie réduit les coûts d'exécution, améliore la stabilité des automatisations et limite la fragilité des scripts face aux évolutions des interfaces web.
---
...
*   ❌ Écrire un script Playwright complexe alors qu'une interface IA native ou une API officielle permet d'effectuer la même opération de manière plus fiable.
