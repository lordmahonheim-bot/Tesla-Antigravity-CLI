...
Aucune capacité n'est activée sans un besoin qualifié.
Le non-respect de cette doctrine compromet la sécurité et la stabilité du système.
...
**De plus, selon la Vigilum Gateway V2.1, toute capacité (Skill) injectée doit déclarer :**
- Ses **dépendances d'outils** (`tool_dependencies`).
- Son **mode de permission requis** (interactive ou goal).
- Son **circuit breaker de retry** (max 3 tentatives pour le self-healing).
...
- Memory Policy
- Security Policy
- tesla-loop-orchestrator (status: Validated)
- tesla-code-auditor (status: Validated)
...
- une opération réseau critique (tel qu'un push distant vers un dépôt GitHub) est planifiée. Tout push distant (y compris sous mode autonome `/goal`) requiert la permission explicite préalable de Lord Mahonheim ;
