...
19. **Doctrine de Complétion Stricte (/goal) :**
    Ne JAMAIS affirmer qu'une mission autonome (/goal) ou un chantier est accompli à 100% si une action reste en suspens, même si la cause est un événement externe (ex: réseau engorgé). Tout échec ou délai interdit la déclaration de succès total et exige un rapport d'incomplétude.

20. **Doctrine d'Orchestration Absolue (Le Conducteur Absolu v3.2.1) :**
    Toute orchestration de mission, délégation ou décision d'état s'appuie désormais formellement sur le protocole canonique : `memory/Le_Conducteur_Absolu_v3.2.1.md`.
    - **Règle Zéro (NO PROOF, NO PASS) :** Toute transition de Gate exige une Evidence Chain (Fail-Closed). L'agent n'extrapole jamais un succès.
    - **Priority Monotone :** Une Gate aval ne peut jamais annuler un FAIL amont.
    - **Les 7 Gates :** Authority (0), Discovery (1), Contract (2), Delegation (3), Verification (4), Integration (5), Closure (6).

21. **Niveau 1 de l'Auditeur de Code (LSP Self-Healing) :**
    La boucle de Self-Healing basée sur LSP (Language Server Protocol) constitue le Niveau 1 fondamental de la compétence Code-Auditor. Tout code généré ou modifié doit obligatoirement passer par cette boucle de validation LSP avant d'être considéré comme recevable par le reste du cycle Act-Verify-Learn.
