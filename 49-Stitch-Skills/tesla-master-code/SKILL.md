...
### 12.2 Self-Healing Loop (SIA-TESLA-H Phase 2)
When validation fails, Master Code must:

1. reproduce the failure using `lsp_diagnostics` (via karellen-lsp-mcp);
2. isolate the cause;
3. assess blast radius;
4. apply the smallest correct fix (Harness modification is STRICTLY FORBIDDEN);
5. rerun the affected stage;
6. rerun the broader pipeline as needed.

> **SIA-TESLA-H Constraint**: `max_retries` is strictly fixed to 3. If validation fails after 3 retries, Master Code must ESCALATE and stop.
...
### 21.1 Contre-Rationalisation / Red Flag : Impression CSS @media print (Gabarit A4)

> [!WARNING]
> Pour forcer un PDF A4 sur 1 page, l'utilisation de `vh`, `height: auto` ou de petits `padding` est une illusion psychologique. 

Master Code DOIT impérativement utiliser des dimensions physiques strictes pour le moteur PDF de Chromium :
- Forcer la dimension de la page avec `@page { size: A4 portrait; margin: 0; }`.
- Imposer que le conteneur global ait une hauteur absolue `height: 297mm` et `width: 210mm`.
- Utiliser des dimensions physiques strictes par bloc : `height: 14cm` (ou `130mm` max par bloc).
- Appliquer impérativement `overflow: hidden !important`, `page-break-inside: avoid` et `-webkit-print-color-adjust: exact`.

### 21.2 Contre-Rationalisation / Red Flag : Physique 3D CSS (Faces de Cartes)

> [!WARNING]
> L'effondrement de la matrice 3D survient souvent si les faces avant/arrière ne sont pas verrouillées.

Master Code DOIT appliquer un verrouillage absolu :
- `position: absolute; width: 100%; height: 100%; top: 0; left: 0;` obligatoire pour les faces (`.front`, `.back`) de toute carte en rotation 3D.

### 21.3 Contre-Rationalisation / Red Flag : Injection DOM et Balises

> [!WARNING]
> Les fuites de code HTML brut (ex: afficher `<strong>Texte</strong>` littéralement) sont des erreurs de manipulation du DOM.

Master Code DOIT adapter la méthode d'injection au type de contenu :
- Interdiction d'utiliser `.textContent` si le contenu à injecter contient des balises HTML (ex: `<br>`, `<strong>`).
- Toujours assainir le contenu et utiliser `.innerHTML` (ou des méthodes sécurisées d'assemblage DOM) pour préserver le rendu HTML.
...
## Appendix: Sub-Skills & References

The following specialized sub-skills are physically integrated into this agent's knowledge base. You must consult these documents when performing related tasks:

- **React Components**: [references/react-components/SKILL.md](references/react-components/SKILL.md)
- **React Native**: [references/react-native/SKILL.md](references/react-native/SKILL.md)
- **React Vite Dashboard**: [references/react-vite-dashboard/SKILL.md](references/react-vite-dashboard/SKILL.md)
- **Shadcn UI**: [references/shadcn-ui/SKILL.md](references/shadcn-ui/SKILL.md)
- **Design MD**: [references/design-md/SKILL.md](references/design-md/SKILL.md)
