...
**Date** : 2026-09-24
**Statut** : [ALIGNÉ SUR L'AUDIT DE CONFRONTATION]
...
Suite au rejet en bloc du brouillon v2.0 par l'**Audit de Confrontation** (pour cause d'incomplétude, de diagnostics fabriqués et de risques systémiques majeurs), la décision souveraine **"Option B — Cannibaliser v2.0"** est adoptée.

Ce document v3.0 n'a **pas** vocation à remplacer le plan v1 (282 Ko, 60 étapes). Le Plan v1 reste le **seul document d'exécution de référence**. Ce v3.0 agit comme un **Patch d'Apports** : il liste exclusivement les 5 contributions techniques valides du v2.0 à injecter dans le v1, et prononce l'annulation définitive des aberrations du v2.0.
...
*Précautions* : La variable `GITLEAKS_LICENSE` peut être requise. `continue-on-error: false` sur le scanner local pour éviter un SPOF réseau si l'action Gitleaks est indisponible.
...
### 2.5 — Hooks natifs correctement câblés (→ Remplace `É-052` v1)
Au lieu des liens symboliques ou du pointage direct (qui plante Git), créer un dossier versionné `.githooks/` contenant **deux wrappers exécutables** :
```bash
...
- 🛑 **INTERDIT (M-07)** : Appliquer un fail-open sur `lint_all.sh` (succès sans linter). **Appliquer la sémantique de l'étape `É-019` du v1 (outil absent = UNAVAILABLE, pas SUCCESS).**
...
***
**Document instancié sous la supervision du Conducteur Absolu.**  
*NO PROOF, NO PASS. Le v3.0 valide la primauté des preuves empiriques de l'Audit de Confrontation.*
